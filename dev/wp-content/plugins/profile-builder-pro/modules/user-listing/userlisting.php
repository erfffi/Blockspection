...
function wppb_generate_userlisting_merge_tags( $type ){
	...
	return apply_filters( 'wppb_userlisting_get_merge_tags', $merge_tags, $type );
}
	...
	foreach ( $userlisting_posts as $key => $value ){
		if ( trim( Wordpress_Creation_Kit_PB::wck_generate_slug( $value->post_title ) ) == $name ){
			...
			if( !empty( $userID ) ){
				...
				return apply_filters( 'wppb_all_userlisting_template', '<div class="wppb-userlisting-container">'.(string) new PB_Mustache_Generate_Template( wppb_generate_mustache_array_for_user_list(), $userlisting_template, array( 'userlisting_form_id' => $value->ID, 'meta_key' => $meta_key, 'meta_value' => $meta_value, 'include' => $include, 'exclude' => $exclude, 'single' => false ) ) . '</div>' ) ;
            }
		}
	}
}
		...
		if ( $wppb_manage_fields != 'not_found' ){		
			...
			foreach( $wppb_manage_fields as $key => $field_value ){
				if ( $name == 'sort_'.$field_value['meta-name'] ){
					...
					return '<a href="'.wppb_get_new_url( $field_value['meta-name'], $extra_info ).'" class="sortLink" id="sortLink'.$i.'">'.$field_value['field-title'].'</a>';
				}
			}
		}
	}
	...
	if( $name == 'users' ){
        ...
        if( !empty( $userlisting_args[0] ) ){
            ...
            if ( isset( $_REQUEST['setSortingOrder'] ) && ( trim( $_REQUEST['setSortingOrder'] ) !== '' ) )
                $sorting_order = $_REQUEST['setSortingOrder'];
            else
            ...
            if ( isset( $_REQUEST['setSortingCriteria'] ) && ( trim( $_REQUEST['setSortingCriteria'] ) !== '' ) )
                $sorting_criteria = $_REQUEST['setSortingCriteria'];
            else
                        ...
                        if( isset( $_GET['ul_filter_'.$faceted_setting['facet-meta']] ) ){
                            if( $faceted_setting['facet-type'] == 'range' ){
                                $args['meta_query'][0][$faceted_setting['facet-meta']] = array(
                                    ...
                                    'value' => explode( '-', $_GET['ul_filter_'.$faceted_setting['facet-meta']] ),
                                    'compare' => 'BETWEEN',
                                    'type'    => 'NUMERIC'
                                );
                            }
                                ...
                                $args['meta_query'][0][$faceted_setting['facet-meta']] = array(
                                    'key' => $faceted_setting['facet-meta'],
                                    'value' => $_GET['ul_filter_'.$faceted_setting['facet-meta']],
                                    'compare' => 'LIKE'
                                );
                            }
                                ...
                                if( wppb_check_if_field_is_multiple_value_from_meta_name( $faceted_setting['facet-meta'], $wppb_manage_fields ) ){
                                    ...
                                    $compare = '=';
                                    $val = $_GET['ul_filter_'.$faceted_setting['facet-meta']];
                                }
                                ...
                                $values = explode( '||', $_GET['ul_filter_'.$faceted_setting['facet-meta']] );
                                if( !empty( $values ) ) {
                                    /* for fields types that have multiple values (checkbox..) we check for the options in the fields settings and not what is stored in the database  */
			...
			if ( isset( $_REQUEST['searchFor'] ) ) {
                $search_for = esc_attr($_REQUEST['searchFor']);
                    ...
                    foreach ($user_meta_keys as $user_meta_key) {
                        if( !in_array($user_meta_key, $wppb_exclude_search_fields ) ) {
                            $args['meta_query'][2][] = array(
                                'key' => $user_meta_key,
                                'value' => apply_filters( 'wppb_ul_search_all_meta_value', $search_for ),
    ...
    if( !empty( $userlisting_args ) ){
        ...
        if ( isset( $_REQUEST['searchFor'] ) ) {
            ...
            if( strpos( preg_replace( '/\s+/', ' ', $query->query_where ), ") ) ) AND (ID = " ) !== false ){
                ...
                $query->query_where = str_replace( "user_nicename LIKE '%".esc_attr($_REQUEST['searchFor'])."%')", "user_nicename LIKE '%".esc_attr($_REQUEST['searchFor'])."%'  OR user_registered LIKE '%". esc_attr($_REQUEST['searchFor']) ."%' OR display_name LIKE '%". esc_attr($_REQUEST['searchFor']) ."%' ) ) ) )", $query->query_where );
            }
            else{
                $query->query_where = str_replace( ") ) AND (ID = ", "OR (ID = ", preg_replace( '/\s+/', ' ', $query->query_where ) );
                /* we add the user_registered column here as well */
...
function wppb_ul_faceted_checkboxes( $faceted_filter_options, $meta_values, $wppb_manage_fields ){
    ...
    if( !empty( $meta_values ) ){
        ...
        foreach( $meta_values as $meta_value => $repetitions ){
            ...
            $filter .= '<label><input type="checkbox" class="wppb-facet-checkbox" value="'. esc_attr( $meta_value ) .'" data-current-page="'. esc_attr( get_query_var('page') ) .'" data-filter-behaviour="'. esc_attr( $faceted_filter_options['facet-behaviour'] ) .'" data-meta-name="'. esc_attr( $faceted_filter_options['facet-meta'] ) .'" '. wppb_ul_checked( $meta_value, $current_value ) .'>';
            $filter .= esc_html( wppb_ul_facet_value_or_label( $meta_value, $faceted_filter_options, $wppb_manage_fields ) ).'( '. $repetitions .' )</label>';
            $filter .= '</div>';
 ...
 * @return string
 */
function wppb_ul_faceted_select($faceted_filter_options, $meta_values, $wppb_manage_fields ){
    $current_value = wppb_ul_get_current_filter_value( $faceted_filter_options['facet-meta'] );

    if( !empty( $meta_values ) ){
        $filter = '<select class="wppb-facet-select" data-filter-behaviour="'. esc_attr( $faceted_filter_options['facet-behaviour'] ) .'" data-current-page="'. esc_attr( get_query_var('page') ) .'" data-meta-name="'. esc_attr( $faceted_filter_options['facet-meta'] ) .'">';
        $filter .= '<option value="">'. __( 'Choose...', 'profile-builder' ) .'</option>';
        foreach( $meta_values as $meta_value => $repetitions ){
            $filter .= '<option value="'.esc_attr( $meta_value ).'" '. wppb_ul_selected( $meta_value, $current_value ) .'>'.esc_html( wppb_ul_facet_value_or_label( $meta_value, $faceted_filter_options, $wppb_manage_fields ) ).'( '. $repetitions .' )</option>';
        }
        $filter .= '</select>';
...
function wppb_ul_faceted_range( $faceted_filter_options, $meta_values, $wppb_manage_fields ){
    ...
    if( !empty( $meta_values ) ) {
        ...
        if( !empty( $meta_values ) ) {
            ...
            if (empty($first_value) || empty($last_value) || empty($first_current_value) || empty($last_current_value))
                return '';
...
/**
 * Function that displays the Label from the Manage fields instead of the data base value if we have one
 * @param $meta_value the database value
 * @param $faceted_filter_options the current filter opstions
    ...
    if( !empty( $wppb_manage_fields ) ){
        foreach( $wppb_manage_fields as $field ){
            if( $field['meta-name'] == $faceted_filter_options['facet-meta'] ){
                if( !empty( $field['labels'] ) || in_array( $field['field'], array( 'Select (Country)' ) ) ){
...
function wppb_ul_get_current_filter_value( $filter_name ){
    if( !empty( $_GET['ul_filter_'. $filter_name] ) )
        $current_value = $_GET['ul_filter_'. $filter_name];
    else
        $current_value = '';
    ...
    if( !empty( $faceted_filters_options ) ){
        ...
        foreach( $faceted_filters_options as $faceted_filter_options ){
            if( isset( $_GET['ul_filter_'.$faceted_filter_options['facet-meta']]  ) ) {
                ...
                $filter_values = explode( '||', $_GET['ul_filter_'.$faceted_filter_options['facet-meta']] );
...
function wppb_userlisting_extra_search_all_fields( $value, $name, $children, $extra_info ){	
	...
	if ( isset($_REQUEST['searchFor'] ) )
		if ( trim( $_REQUEST['searchFor'] ) != $searchText )
			$searchText = esc_attr( trim( $_REQUEST['searchFor'] ) );
...
function wppb_manage_ul_cpt(){
    ...
    $facet_types = array( '%Checkboxes%checkboxes', '%Select%select', '%Range%range', '%Search%search' );
    $facet_meta = array();
